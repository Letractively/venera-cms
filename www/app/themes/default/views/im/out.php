<h2><?php echo __l('im',lt('im_inmessages')).' | '.lt('im_outmessages');?></h2>
<?php 
...
foreach($msg_list as $i=>&$val){
	$userpic=(file_exists('uploads/avatar_'.$val['uID'].'.jpg')?BASEURL.'uploads/avatar_'.$val['toID'].'.jpg':THEME.'img/default_avatar_'.$val['gender'].'.jpg');
	...
	<small>'.lt('im_to').': '.$val['name'].' '.$val['login'].' '.$val['sername'].'<br/>'.date('d.m.Y H:i:s',$val['datetime']).'</small>
	</div>
	<u>'.$val['theme'].'</u><br/><small>'.substr($val['text'],0,200).'</small><div style="clear:both;"></div>
	<a href="'.l('im/history/'.$val['toID']).'">['.lt('im_view').'/'.lt('im_newmsg').']</a><br/>'
...
<br/>
<?php echo venus_nav_links('im/out/{%page%}',array('pagenow'=>$pNum,'maxpage'=>ceil($msg_count/$pCount)-1));?>